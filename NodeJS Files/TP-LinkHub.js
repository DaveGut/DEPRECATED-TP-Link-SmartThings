...
function discoverTPLinkDevices() {
	...
	socket.on('message', function (msg, rinfo) {
		...
		if (!device.model) { device = device.system }

		var model = device.model.substring(0,5)
		var tplinkDNI
		if (device.mic_type == "IOT.SMARTBULB") {
			tplinkDNI = device.mic_mac
		} else if (device.type == "IOT.RANGEEXTENDER.SMARTPLUG") {
			tplinkDNI = device.ethernet_mac.replace(/:/g, "")
		} else {
		...
		var tplinkDevice = {}
		if (device.child_num) {
			var totalPlugs = device.child_num
