...
def setInitialStates() {
	...
	if (!state.flowType) { state.flowType = null }
	...
	return dynamicPage (name: "welcomePage", 
        ...
        if (installType == "Kasa Account") {
 		...
 		section("Update Installation Data", hideable: true, hidden: true) {
			...
			href "welcomePage", title: "Update Device Data (Upgrade Only)", description: "Tap to Update Installation Data"
		}
...
def addDevicesPage() {
	...
	return dynamicPage (name: "addDevicesPage", 
        ...
        errorSection()
        section() { 
        	paragraph "On initial installation, you must install at least one device or app will not install!"
        }
  		section("Select Devices to Add (${newDevices.size() ?: 0} found)") {
			input ("selectedAddDevices", "enum", 
                    ...
                    title: null, 
                    options: newDevices)
			paragraph "Select text at upper left corner to install devices and the Application."
			paragraph "Select  '<'  at upper left corner to exit."
        }
	}
}
	...
	return dynamicPage (name: "devicePreferencesPage", 
		...
		section("Device Configuration: ") {
			...
			if (selectedUpdateDevices) {
				...
				href "welcomePage", title: "Update Devices", description: "Tap to continue"
            }
			paragraph "Select  '<'  at upper left corner to exit."
		}
	}
